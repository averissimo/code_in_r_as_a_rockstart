...
## _{{< fa solid code-branch >}} git_ repositories to track code {.smaller background-color="#3b1200"}
...
## Write readable code {background-color="#3b1200" .smaller}
...
- `|>` Pipe operator
    - shortcut in rstudio: _[ctrl + shift + m]{.highlight-soft}_
- `%||%`
- `sapply` _vs._ `for (x in z) { ... }`
- `Filter()` / `Reduce()`
...
## `sapply()` / `vapply()` / ... {background-color="#191e31" .smaller}
...
## `mclapply()` {background-color="#191e31" .smaller}
...
## `Reduce()` / `Filter()` {background-color="#191e31" .smaller}
...
## `local()` {background-color="#191e31" .smaller}

::: aside
{{< fa solid circle-info >}}  `<<-` allows to write to parent environment _(use with care)_
:::
...
## `📦dplyr` {background-color="#1a162d" .smaller .scrollable}
...
## `📦tidyr` {background-color="#1a162d" .smaller}
...
## `📦purrr` {background-color="#1a162d" .smaller}
...
## `📦glue` {background-color="#1a162d" .smaller}
...
Data can come from other sources, such as a _dataframe_ _(a list or an environment)_
...
## `📦ggplot2` (source [gist](https://gist.github.com/averissimo/8804d3ffed0613f9f912c4ef3465fbc1))  {background-color="#1a162d" .smaller}
...
# 💣 Don't `source` files {background-color="#6877a1" .smaller}
...
**[If you really need]{.highlight}**, use relative paths:
  ...
  - `here::here("./another.R")` _(in current directory)_
  - `here::here("../something.R")` _(in parent directory)_
  - _or_ `here::here("./folder/to/script.R")` _(in one of the folders)_

## Alternatives to `source()` {background-color="#6877a1" .smaller}
...
- The file can be loaded and then used with its name
`<filename>$<fun_name>`
...
&nbsp; _(let's try a live demo if we have time)_

# Thanks!

::: aside
[_Take some cool Appsilon's package stickers with you_]{.highlight-soft}
:::
